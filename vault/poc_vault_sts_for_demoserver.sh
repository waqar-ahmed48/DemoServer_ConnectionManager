...
if [[ -n "$VAULT_COMMAND_OUTPUT" && "$VAULT_COMMAND_OUTPUT" != "null" ]]; then
  ...
  VAULT_COMMAND_OUTPUT=$(vault write -tls-skip-verify $DEMOSERVER_AWS_ENGINE_PATH/roles/DemoUser credential_type=session_token)
  echo Create SessionToken Role in AWS Secrets Engine: $VAULT_COMMAND_OUTPUT

  VAULT_COMMAND_OUTPUT=$(vault read -tls-skip-verify $DEMOSERVER_AWS_ENGINE_PATH/sts/DemoUser  ttl=900s)
  ...
  export AWS_SESSION_TOKEN=$(echo "$VAULT_COMMAND_OUTPUT" | grep 'session_token' | awk '{print $2}')
